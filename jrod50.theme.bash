...
USER_COLOR="${bold_red}"
SUPERUSER_COLOR="${bold_red}"
MACHINE_COLOR=${bold_blue}
IP_COLOR=$ORANGE
...
function prompt() {
    ...
    if [[ $VIRTUAL_PROMPT_ENABLED == 1 ]]; then
        PS1="$(clock_char)  $(scm_char) ${UC}\u ${DEFAULT_COLOR}at ${MACHINE_COLOR}\h$(ip_prompt_info) ${DEFAULT_COLOR}in ${DIRECTORY_COLOR}$(limited_pwd)${DEFAULT_COLOR}$(virtual_prompt_info)$(scm_prompt_info)${reset_color} \$ "
    else
